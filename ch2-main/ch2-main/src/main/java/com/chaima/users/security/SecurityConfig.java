package com.chaima.users.security;
...
.cors().configurationSource(new CorsConfigurationSource() {
	...
	config.setAllowedOrigins(Collections.singletonList("http://localhost:4200"));
	 ...
	 return config;
		 ...
		 .requestMatchers("/deleteUserById/{id}").permitAll()
		 .requestMatchers("/findRoleById/{id}").permitAll()
		 .requestMatchers("/removeRoleFromUer/{id}").permitAll()
